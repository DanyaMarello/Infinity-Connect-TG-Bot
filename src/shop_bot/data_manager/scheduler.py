...
async def check_expiring_subscriptions(bot: Bot):
    ...
    current_time = time_utils.now_utc()
    ...
    for squad in squads:
        ...
        for db_key in keys_in_db:
            ...
            try:
                ...
                if expiry_date and getattr(expiry_date, 'tzinfo', None) is None:
                    expiry_date = expiry_date.replace(tzinfo=timezone.utc)
            except Exception:
                try:
            ...
            if remote_user:
                ...
                if expire_value:
                    try:
                        ...
                        if remote_dt and getattr(remote_dt, 'tzinfo', None) is None:
...
async def _send_alert(bot: Bot, scope: str, name: str, issues: list[dict], level: str):
    ...
    text_lines = [
        ...
        f"⏰ <b>Время:</b> <code>{time_utils.now().strftime('%d.%m.%Y %H:%M:%S')}</code>",
        "",
        "📊 <b>Проблемы:</b>"
    ]
