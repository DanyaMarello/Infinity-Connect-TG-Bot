...
def _now_str() -> str:
    return time_utils.now_utc().strftime("%Y-%m-%d %H:%M:%S")
...
def create_gift_key(user_id: int, host_name: str, key_email: str, months: int, remnawave_user_uuid: str | None = None) -> int | None:
    ...
    try:
        ...
        expiry_dt = time_utils.now_utc() + timedelta(days=30 * months_value)
        expiry_ms = int(expiry_dt.timestamp() * 1000)
        uuid_value = remnawave_user_uuid or f"GIFT-{user_id}-{int(time_utils.now_utc().timestamp())}"
        return add_new_key(
            user_id=user_id,
            host_name=host_name,
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            if not row:
                ...
                cursor.execute(
                    ...
                    (telegram_id, username, time_utils.now_utc().strftime("%Y-%m-%d %H:%M:%S"), referrer_id)
                )
            else:
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            cursor.execute(
                """INSERT INTO transactions
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (username, transaction_id, payment_id, user_id, status, amount_rub, amount_currency, currency_name, payment_method, metadata, time_utils.now_utc().strftime("%Y-%m-%d %H:%M:%S"))
            )
            conn.commit()
    except sqlite3.Error as e:
