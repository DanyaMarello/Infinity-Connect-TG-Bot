...
def _connect() -> sqlite3.Connection:
    # Настройка соединения с SQLite: включаем WAL и увеличиваем таймаут для лучшей конкурентности
...
def _default_expire_at_ms() -> int:
    return int(time_utils.now_utc().timestamp() * 1000)
    ...
    with _connect() as conn:
        ...
        if expires_at:
            try:
                ...
                exp_dt = None
            if exp_dt and exp_dt < time_utils.now_utc():
                return None
