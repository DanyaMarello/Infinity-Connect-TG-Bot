        ...
        for i, key in enumerate(keys):
            ...
            try:
                if expiry_date.tzinfo is None:
                    expiry_date = expiry_date.replace(tzinfo=timezone.utc).astimezone(time_utils.TZ)
                else:
                    expiry_date = expiry_date.astimezone(time_utils.TZ)
            except Exception:
                expiry_date = time_utils.now()
            status_icon = "✅" if expiry_date > time_utils.now() else "❌"
