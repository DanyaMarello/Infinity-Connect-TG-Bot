...
def get_user_router() -> Router:
    ...
    async def profile_handler_callback(callback: types.CallbackQuery):
        ...
        now = time_utils.now()
        def _parse_key_expiry(k):
            try:
                d = datetime.fromisoformat(k['expiry_date'])
                if d.tzinfo is None:
                    d = d.replace(tzinfo=timezone.utc).astimezone(time_utils.TZ)
                else:
                    d = d.astimezone(time_utils.TZ)
                return d
            except Exception:
                return time_utils.now()

        active_keys = [key for key in user_keys if _parse_key_expiry(key) > now]
        if active_keys:
            latest_key = max(active_keys, key=lambda k: _parse_key_expiry(k))
            latest_expiry_date = _parse_key_expiry(latest_key)
    ...
    async def topup_pay_tonconnect(callback: types.CallbackQuery, state: FSMContext):
        ...
        transaction_payload = {
            ...
            'valid_until': int(time_utils.now_utc().timestamp()) + 600
        }
    ...
    async def process_trial_key_creation(message: types.Message, host_name: str):
        ...
        try:
            ...
            while True:
                ...
                if attempt > 100:
                    candidate_local = f"{base_local}-{int(time_utils.now_utc().timestamp())}"
                    candidate_email = f"{candidate_local}@bot.local"
                    break
...
async def process_successful_payment(bot: Bot, metadata: dict):
    ...
    try:
        ...
        if action == "extend":
            try:
                # Попробуем извлечь текущую дату окончания в удобном формате
                cur_exp = existing_key.get('expiry_timestamp_ms') or existing_key.get('expiry_ms') or existing_key.get('expire_at') or existing_key.get('expiry_date') or existing_key.get('expire_at_ms')
            except Exception:
                cur_exp = None
        ...
        if expiry_ts_param is not None:
            result = await remnawave_api.create_or_update_key_on_host(
                host_name=host_name,
                email=candidate_email,
                expiry_timestamp_ms=expiry_ts_param
            )
        else:
            result = await remnawave_api.create_or_update_key_on_host(
                host_name=host_name,
        ...
        final_text = get_purchase_success_text(
            ...
            expiry_date=new_expiry_date or time_utils.now(),
