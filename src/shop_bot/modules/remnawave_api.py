...
def _normalize_email_for_remnawave(email: str) -> str:
    ...
    if not local or not re.match(r"^[a-z0-9]", local):
        local = f"u{local}" if local else f"user{int(time_utils.now_utc().timestamp())}"
...
def _normalize_username_for_remnawave(name: str | None) -> str:
    ...
    if not base or not re.match(r"^[a-z0-9]", base):
        base = f"u{base}" if base else f"user{int(time_utils.now_utc().timestamp())}"
    if len(base) > 32:
        base = base[:32].rstrip("_-") or base[:32]

    if len(base) < 3:

        suffix = str(int(time_utils.now_utc().timestamp()))
        base = (base + suffix)[:3]
    ...
    *,
    ...
    try:
        ...
        if expiry_timestamp_ms is not None:
            ...
            if days <= 0:
                days = 1
            target_dt = time_utils.now_utc() + timedelta(days=days)
