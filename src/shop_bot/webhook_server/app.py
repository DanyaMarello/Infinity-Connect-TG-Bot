...
def create_webhook_app(bot_controller_instance):
    ...
    def inject_current_year():
        ...
        return {
            'current_year': time_utils.now().year,
    ...
    def create_key_ajax_route():
        ...
        if mode == 'personal':
            ...
            if expiry_ms is None and days_total > 0:
                expiry_ms = int((time_utils.now_utc() + timedelta(days=days_total)).replace(tzinfo=timezone.utc).timestamp() * 1000)
    ...
    def adjust_key_expiry_route(key_id: int):
        ...
        try:
            ...
            if isinstance(cur_expiry, str):
                try:
                    ...
                    try:
                        ...
                        exp_dt = time_utils.now_utc()
            else:
                exp_dt = cur_expiry or time_utils.now_utc()
            ...
            try:
                ...
                new_dt_local = time_utils.from_utc_ms(new_ms_final).astimezone(time_utils.TZ)
                text = (
                ...
                logger.exception("Не удалось уведомить пользователя о новом ключе")
    ...
    def sweep_expired_keys_route():
        ...
        for k in keys:
            ...
            try:
                ...
                logger.exception("Ошибка обработки exp_dt при проверке истечения ключа")
        ...
        try:
            ...
            if existing:
                ...
                if not (filename.endswith('.zip') or filename.endswith('.db')):
                    ...
                    return redirect(request.referrer or url_for('settings_page', tab='panel'))
                ts = time_utils.now().strftime('%Y%m%d-%H%M%S')
    ...
    def test_webhook():
        ...
        if request.method == 'GET':
            return f"Webhook server is running! Time: {time_utils.iso_now()}"
        else:
